...
# Vamos avaliar os gastos com combustíveis e lubrificantes. Começando pela evolução anual destes gastos.
...
plot_bar(gasto_anual_combs_lubr, "Total de gastos anuais com combustíveis e lubrificantes",
        "Ano", "Gasto", 'yellow')


# A partir de 2009 (período onde o total com gastos em combustíveis e lubrificantes foi de praticamente 14MM) podemos observar um significante aumento em 2010 e 2011, uma queda entre 2012 e 2014 e um novo aumento em 2015, quando estes gastos ultrapassaram a casa dos 15 MM e saltaram para mais de 17MM.
# 
# O que teria influenciado na redução dos gastos entre 2012 e 2014 e seus substancial aumento em 2015? Eu tenho algumas hipóteses:
# 
# **1. Houve uma mudança no comportamento dos parlamentares em relação a estes gastos?;**
# 
# 
# 
# **2. O valor do combustível esteve menor neste período?**
# 
# Vamos testar essas possibilidades. Para responder a primeira, parece óbivio observar se a quantidade de pedidos de reembolsos relativos a este gasto diminuiu. Já para a segunda hipótese, vamos buscar o preço médio do combustível(gasolina e etanol) no período em que ocorreu a queda. 
# 
# 
# 
# 
...
reemb_ano = df_combustiveis.ano_despesa.value_counts()
plot_bar(reemb_ano)


# De acordo com o gráfico acima, parece ter ocorrido um comportamento similar em relação aos pedidos dos parlamentares para este tipo de reembolso.
# 
# Mas será que isso já explica a queda entre 2012 e 2014 ? 
# 
# Para ir mais a fundo nessa questão, vamos obter o valor da correlação entre o número de pedidos e o valor de reembolso ao longo dos anos. 
# 
# Para isso vamos criar um mini dataset com estes dados:

# In[59]:


# Transformando os pedidos anuais de reembolso em um serie do pandas

serie_reemb_comb_ano = pd.Series(reemb_ano)
serie_reemb_comb_ano.sort_index(inplace=True)

# Criando um novo df com os dados de qtd de pedidos, valor do reembolso e anos

df_gastos_pedidos_comb = round(pd.DataFrame(data=[np.array(serie_reemb_comb_ano.values), np.array(gasto_anual_combs_lubr.values)]))
df_gastos_pedidos_comb = df_gastos_pedidos_comb.T
df_gastos_pedidos_comb.columns = ['pedidos_reemb', 'total_gastos']
df_gastos_pedidos_comb['ano'] = np.arange(2009,2018)
df_gastos_pedidos_comb


# In[60]:


corr_gastos_pedidos_anos = gastos_pedidos_comb.corr()

plt.figure(figsize=(12,10))
sns.heatmap(corr_gastos_pedidos_anos, annot=True, cmap='Blues')

plt.title("Correlação entre os gastos x pedidos x anos x volume de gastos");


# Entendendo este quadro:
#     
# O quadro acima indica a correlação das variáveis total_de gastos, pedidos de reembolso e ano. A correlação de Pearson indica como a alteração em uma variável influencia no comportamento de outra variável. Este valor varia entre -1 e 1. Um valor de -1, indica uma correlação negativa perfeita, ou seja, as variáveis tem uma relação inversa, quando uma aumenta a segunda reduz, ou vice versa. Já uma correlação de 1, indica uma correlação positiva perfeita, as duas variáveis tem movimento na mesma direção. O que este quadro nos mostra é:
# 
# a. A correlação entre os anos e o volume de gastos é positiva. O valor de 0.77, indica que a medida que os anos passam, os gastos tendem a aumentar. Podemos considerar essa correlação como sendo uma correlação positiva forte;
# 
# b. A correlação dos pedidos de reembolso com os anos é de -0.34. Trata-se de uma correlação negativa fraca, mas pode indicar que realmente os pedidos de reembolso referentes a cobustíveis e lubrificantes, estão reduzindo com o passar do tempo;
# 
# c. Já a relação entre os pedidos de reembolso e o volume de gastos, tem uma relação positiva fraca (0.27), que indica que a medida que os pedidos de reembolso aumentam, o volume de gastos também tende a aumentar. 
# 
# 
# Os gráficos a seguir, ilustram de maneira mais clara o que vimos no quadro acima:
...
plt.ticklabel_format(style='plain', axis='y') # Removendo a notação científica

sns.regression.regplot(x='ano', y='total_gastos', data=df_gastos_pedidos_comb)


plt.title("Relação entre gastos x anos");


# A linha do gráfico mostra a clara relação de aumento dos gastos a medida que o tempo passa. 
...
sns.regression.regplot(x='ano', y='pedidos_reemb', data=df_gastos_pedidos_comb)

plt.title("Relação entre ano x pedidos de reembolso");


# Aqui vemos a leve queda dos pedidos de reembolso ao passar do tempo.
...
sns.regression.regplot(x='pedidos_reemb', y='total_gastos', data=df_gastos_pedidos_comb)

plt.title("Relação entre gastos x pedidos de reembolso");



# Aqui, a relação entre os pedidos de reembolso e o aumento dos gastos.
# 
# 
# Mas se os pedidos de reembolso, tem uma relação de queda com o passar do tempo, por que o valor de reembolso não caiu? Isso pode ser excplicado pelo aumento do preço médio do combustível. Segundo a ANP, o valor médio do combustível por litro, considerand gasolina e Etanol, é este: 

# In[ ]:


preco_comb = pd.read_excel('combustivel_preco.xlsx')
sns.barplot(x=preco_comb.ano, y=preco_comb.valor_medio_litro)
plt.title('Preço médio do combustível por ano - Litro');


# In[ ]:


gastos = df_gastos_pedidos_comb.total_gastos[2:9]

d = {'gastos': gastos.reset_index(drop=True), 'preco_comb': preco_comb.valor_medio_litro}
...
sns.regression.regplot(x='preco_comb', y='gastos', data=df)

plt.title("Relação entre preço do combustível x gsstos");


# Acima, vemos claramente como o custo médio do combustível influencia no valor total dos gastos nessa categoria ( o valor da correlação é de 0.76). Por este motivo, que mesmo havendo uma leve relação de queda entre os anos e os pedidos de reembolso, não observamos o mesmo comportamento no volume dos gastos.
# 
# Entretanto, uma redução maior nos pedidos poderia auxiliar a queda desses gastos. 
